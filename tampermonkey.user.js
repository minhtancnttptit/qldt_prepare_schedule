...
document.addEventListener ("DOMContentLoaded", function(){
	...
	var rows = 6; //here's your number of rows and columns, rows is 7, 9 , 12
	var cols = 70;
	var tables = [];
	var m1 = new Map();
	var m2 = new Map();
    ...
    console.log("create tkb_preview_table");
    ...
    console.log('click');
		...
		if(m1.get(subject_code) != undefined){
			var tmp_213 = m1.get(subject_code); // danh sach cac nhom mon hoc
			for(i = 0; i < tmp_213.length; i++){
				if($("#tkb_preview_table1").find('td.bvk_table_cell').eq(tmp_213[i]).html() == "+"){ //dang trung`
		...
		course.eq(16).find('div.top-fline').map(function(){ // col tuan
			// alert($(this).find("label").attr("id").length)
			var tmp_151 = []
			for(i = 0; i < $(this).find("label").attr("id").length; i++){
				if($(this).find("label").attr("id")[i] != "-"){

					if(i < 9){
						tmp_151.push(parseInt($(this).find("label").attr("id")[i]))
					} else if(i < 19) {
						tmp_151.push(parseInt("1" + $(this).find("label").attr("id")[i]))
					} else if(i < 29){
						tmp_151.push(parseInt("2" + $(this).find("label").attr("id")[i]))
					} else if(i < 39){
						tmp_151.push(parseInt("3" + $(this).find("label").attr("id")[i]))
					} else if(i < 48){
						tmp_151.push(parseInt("4" + $(this).find("label").attr("id")[i]))
					} else {
						tmp_151.push(parseInt("5" + $(this).find("label").attr("id")[i]))
					}
				}
			}
            console.log(tmp_151)
      ...
      console.log('test');
        ...
        console.log(tmp_219);
        tmp_219 = [...new Set(tmp_219)];
				for(i = 0; i < tmp_219.length; i++){
          console.log($("#tkb_preview_table1").find("td.bvk_table_cell").eq(tmp_219[i]).text());
        ...
        tmp_219 = [...new Set(tmp_219)];
				for(i = 0; i < tmp_219.length; i++){
          console.log($("#tkb_preview_table2").find('td.bvk_table_cell').eq(tmp_219[i]).text());
